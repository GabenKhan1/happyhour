...
//*============Edit Screen Querry =========*//
  ...
  Stream<List<DocumentSnapshot>> fetchHourInradius(
    ...
    var queryRef = _firebaseFirestore.collection("happyhours");
    String field = 'position';

   return  GeoCollectionReference<Map<String, dynamic>>(queryRef)
        .subscribeWithin(
  center: GeoFirePoint(GeoPoint(lat, long)),
  radiusInKm: 50,
  field: field,
  geopointFrom: (_)=>
  GeoPoint(lat, long),
);
    // return   geo.data (collectionRef: queryRef).within(
  ...
  Future updateHourInFirebase({
    ...
    await _firebaseFirestore.collection("happyhours").doc(docId).update({
      "id": id,
      "businessName": businessName,
      "businessAddress": businessAddress,
      "description": description,
